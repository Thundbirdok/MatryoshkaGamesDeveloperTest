...
namespace CookingPrototype.Controllers {
	public class CustomersController : MonoBehaviour {
		...
		public bool ServeOrder(Order order) {
			var SortedList = CustomerPlaces.Where(x => !x.IsFree).OrderBy(x => x.CurCustomer.WaitTime).ToList();			
			foreach ( var customerPlace in SortedList ) {				
				if ( customerPlace.CurCustomer.ServeOrder(order) ) {					
					if ( customerPlace.CurCustomer.IsComplete ) {
